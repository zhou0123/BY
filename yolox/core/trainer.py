...
class Trainer:
    ...
    def train_one_iter(self):
        ...
        # inps_, targets_ = self.prefetcher.next()
        # if not torch.is_tensor(finps):
        #     finps = inps_
        #     ftargets=targets_
        #     return
        # #import pdb;pdb.set_trace()
        # inps =torch.cat([finps,inps_],dim=0)
        # targets = torch.cat([ftargets,targets_],dim=0)
        # finps = inps_
        # ftargets =targets_
        #print(inps.shape)
        track_ids = targets[:, :, 5]
        ...
        with torch.cuda.amp.autocast(enabled=self.amp_training):
            outputs = self.model(inps, targets,track_ids)
    ...
    def after_epoch(self):
        if self.epoch !=0 and self.epoch%79==0:
            if self.use_model_ema:
